...
namespace PhanMemQLTV
{
    ...
    {
        public static string tkDGDangNhap = "";
        public frmDoiMKDG()
        {
            InitializeComponent();
        }
        ...
        {
            txtTenTaiKhoan.Text = tkDGDangNhap;
        }
        ...
        private void btnDoiMatKhau_Click(object sender, EventArgs e)
        {
            ...
            if (txtTenTaiKhoan.Text.Length > 0 && txtMatKhau.Text.Length > 0 && txtMatKhauMoi.Text.Length > 0 && txtNhapLaiMKMoi.Text.Length > 0)
            {
                try
                {
                    ...
                    myCommand.Parameters.Add(new SqlParameter("@pass", frmConfig.md5_Hash(txtMatKhau.Text)));
                    ...
                    if (x == 1)
                    {
                        if (kq == 0)
                        {
                            string luumk = "update tblDocGia set MatKhauDG='" + frmConfig.md5_Hash(txtMatKhauMoi.Text) + "' where MaDG='" + txtTenTaiKhoan.Text + "'";
                            myConnection = new SqlConnection(chuoiKetNoi);
                            myConnection.Open();
